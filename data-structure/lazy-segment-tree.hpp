...
struct lazy_segment_tree {
    ...
    lazy_segment_tree(const std::vector<T> &src) : n(src.size()), data(n << 1), lazy(n << 1, S::id_E()) {
        ...
        for (int i = n - 1; i > 0; i--) { data[i] = S::op_TT(data[i << 1 | 0], data[i << 1 | 1]); }
    }
    ...
    void apply(int l, int r, const E &x) {
        ...
        while (l >>= 1, r >>= 1) {
            data[l] = S::op_TT(S::op_TE(data[l << 1 | 0], lazy[l << 1 | 0]),
                               S::op_TE(data[l << 1 | 1], lazy[l << 1 | 1]));
            data[r] = S::op_TT(S::op_TE(data[r << 1 | 0], lazy[r << 1 | 0]),
                               S::op_TE(data[r << 1 | 1], lazy[r << 1 | 1]));
        }
    }
    T fold(int l, int r) {
        l += n, r += n - 1;
        ...
        T a = S::id_T(), b = S::id_T();
        for (r++; l < r; l >>= 1, r >>= 1) {
            if (l & 1) { propagate(l), a = S::op_TT(a, data[l++]); }
            if (r & 1) { propagate(--r), b = S::op_TT(data[r], b); }
        }
        return S::op_TT(a, b);
    }
};
